...
- [x] A7 格局识别 API 入口（DetectGrands）—— **2026-09-05 落地**：`DetectGrands(const GridInfo*,
  ...
  **遗留② 单一实现重构 2026-09-05 完成**：DisplayGrands 收为薄壳（DetectGrands + 按记录序
  ...
  No-major」守护防漏打/重打（检测正确性由 §1-8 覆盖）。
  **遗留① 真实盘 golden 对拍 2026-09-05 完成**：发现 golden_main 支持 `-os` 文本 listing
  且 `-g0` 触发格局段 → CLI 加 opt-in `--grands`（FCreateGrid(false)+DetectGrands，默认不
  触发不影响金样）；8 盘 golden `-g0` 计数 fixture 入 `test/golden/*.grands.json`，
...
**`-Ao` 深挖定论（2026-09-05 深夜，追加）**：原版 console 变体实测——名称文法
（`-Ao Conjunct Conjunct 10`）**rc=3 拒绝**（原版 pmAspect 实为数值索引）；数值范围
`-Ao i j v..`：单值（i==j，如 `-Ao 1 1 0.1`→22 行、`-Ao 3 3 0.1`→20 行）**正常**；
但**多值范围（j>i，如 `-Ao 1 5 8 8 8 8 8`）→ 输出退化为 Ari0° 99 行乱表**（值列表
吞掉 -qb 参数、行星被置于 0°Ari）→ **原版 mingw console 端口的 -A 族多值路径不可靠**。
本地引擎单值文法 `-Ao 3 0.1`→18 行、`-Ao 1 0.1`→22 行与数值预期一致（Con/Squ 表精确
对应用行数变化）；**orb 轴金样（原版侧生成）被阻塞**，需改用 config 文件通道或修原版
console 端口后方可继续切片②。CLI 侧已顺带修复：透传改为消费「非 '-' 开头值块」（原先
只吃单个数字 token，`-Ao 6 9.5` 等本地文法经 CLI 从未真正生效）。
